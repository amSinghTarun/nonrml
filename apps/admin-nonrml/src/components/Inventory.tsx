    ...
    const handleInventoryVariantSizeQuantityChange = ({inventoryId, quantity, size}: {inventoryId: number, quantity?: string, size?: string}) => {
        try{
            ...
            setVariantDetails( (prev) => {
                let prevInventoryDetail = { [inventoryId] : {
                    ...prev[inventoryId],
                    ...( (quantity && Number(quantity) >= 0 ) && { "quantity": +quantity } ), 
                    ...( size && { "size": size } )
                }}
                
                quantity?.trim() == "" && delete prevInventoryDetail[inventoryId].quantity;
                size?.trim() == "" && delete prevInventoryDetail[inventoryId].size;
        ...
        <TableBody>
            ...
            inventory.data.data.map( inventoryItem => (
                ...
                <TableCell className="text-xs text-white flex gap-2">
                    <button onClick={ async () => {inventoryItem.quantity == 0 ? (await inventoryAction.mutateAsync({id: inventoryItem.id}), inventory.refetch()): setError("Can't delete inventory item with quantity more th  zero")} } className="p-1 bg-stone-600 rounded-md hover:bg-stone-200 hover:text-black">{inventoryAction.isLoading ? "DELETING" : "DELETE"}</button>
                    {
