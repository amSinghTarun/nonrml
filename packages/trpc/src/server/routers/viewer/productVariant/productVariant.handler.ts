...
import { TAddProductVariantsSchema, TDeleteProductSchema, TEditProductSchema, TGetProductSchema, TGetProductsSchema } from "./productVariant.schema";
...
export const addProductVariants = async ({ctx, input} : TRPCRequestOptions<TAddProductVariantsSchema>) => {
    try{
        ...
        const product = await prisma.productVariants.createMany({
            data: input!
        });
        return {status:TRPCResponseStatus.SUCCESS, message:"Product variants added", data: product};
    } catch(error) {
