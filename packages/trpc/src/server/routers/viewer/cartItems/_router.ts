...
export const cartItemRouter = router({
    ...
    editCartItem: publicProtectedProcedure
        ...
        .mutation( async ({ctx, input}) => await increaseItemQuantityInCart({ctx, input}) ),
    deleteCartItem: publicProtectedProcedure
        ...
        .mutation( async ({ctx, input}) => await deleteItemFromCart({ctx, input}) ),
});